...
function deleteMessage($message, queue) {
  const rndBoundary = Math.random().toString(32).substr(-8)+Math.random().toString(32).substr(-8);
...
function onRequestDone() {
  if (state.count.requesting === 0) {
    setTimeout(() => { TIMEOUT_REQUEST = 1000 }, 2000); // reset
  }
}

function hasMoreMassage() {
  return document.querySelector('#end_display_status').textContent.trim().indexOf('And more') > -1;
}
...
function deleteAll() {
  ...
  const collectAndCheck = () => {
    ...
    getLatest(queue, REQUEST_RATE_LIMITING - state.count.requesting).forEach($message => deleteMessage($message, queue));
